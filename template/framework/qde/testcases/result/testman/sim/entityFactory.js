...
function getEntityTree(type, nodeFn) {
	...
	function isSameNode(a, b) { return a.id == b.id && a.nodeType == b.nodeType; }
	function getParentEntity(itemId) {
		...
		else if (!IX.Array.isFound(node, pnode.children, isSameNode))
...
function queryEntities(type, params) {
	...
	if ('pageSize' in params) {
		...
		var stx = pageNo * pageSize; 
		var endx = Math.min(stx + pageSize, itemIds.length);
		return {
...
var entityFactory = {
	registerTypes: function (typeHT) { TypedHT = typeHT; },
	...
	getEntityTree4Select: getEntityTree4Select
};

/* eslint-disable */
if (IX_DOM_MODE) { // used in DOM 
	IX.ns('Test');
	Test.entityFactory = entityFactory;
	return;
}
// used in NODE
